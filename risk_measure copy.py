...
def adjProbStrategicVoting(
    _: np.ndarray,
    __: VotingScheme | None,
    voterHappiness: list[float],
    strategicOptionsWithCounters: list[set[tuple[np.chararray, float, set[tuple[np.chararray, float, float]]]]],
):
    n = len(voterHappiness)
    count = 0.0

    for i, strategic_options in enumerate(strategicOptionsWithCounters):
        for strategic_option, mod_hapiness, counters in strategic_options:
            if mod_hapiness > voterHappiness[i] and counters:
                if not counters:  # If there are no counter options
                    count += 1
                    break

                is_countered = False
                for counter_strategic_options in counters: # For every counter voter
                    for counter_option, mod_happiness_after_counter, counter_mod_happiness, counter_happiness in counter_strategic_options:
                        if counter_mod_happiness > counter_happiness and mod_happiness_after_counter < voterHappiness[i]:
                            is_countered = True
                            break
                    if is_countered:
                        break
...
def naiveVoting(
    ...
    staretgic_voting_probs = np.array([])

    for i, strategic_options in enumerate(strategicOptionsWithCounters):
        n = len(strategic_options)
        count = 0.0

        for strategic_option, mod_hapiness in strategic_options:
            if mod_hapiness > voterHappiness[i]:
                count += 1

        staretgic_voting_probs.add(count / n)

    return staretgic_voting_probs.mean()
    ...
    for i, strategic_options in enumerate(strategicOptionsWithCounters):
        ...
        for strategic_option, mod_hapiness, counters in strategic_options:
            if mod_hapiness > voterHappiness[i]:
                ...
                count_counters = 0
                for counter_option, counter__mod_happiness, counter_happiness in counters:
                    if counter__mod_happiness > counter_happiness:
                        count_counters += 1

                count += count_counters / n_counters

        staretgic_voting_probs.append(count / n)

    return staretgic_voting_probs
